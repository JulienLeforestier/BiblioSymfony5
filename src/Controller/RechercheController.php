...
class RechercheController extends AbstractController
{
    ...
    public function search(Request $request, LivreRepository $livreRepository, AbonneRepository $abonneRepository): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            $liste_livres = $livreRepository->findBySearch($search);
            $liste_livres_indispos = $livreRepository->findByAvailable();
            if (count($liste_abonnes) < 1 && count($liste_livres) < 1) $this->addFlash("danger", "Aucun abonné ou livre ne correspond à votre recherche");
            return $this->render('recherche/search.html.twig', compact('liste_abonnes', 'liste_livres', 'liste_livres_indispos', 'search'));
        }
